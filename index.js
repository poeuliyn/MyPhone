...
function openPhone() {
 ...
 overlay.style.display = 'flex'; // สำรองไว้เผื่อ style.css โหลดไม่สำเร็จ ไม่พึ่ง class อย่างเดียว
...
function buildSkeleton() {
 ...
 // ★ ฝังสไตล์จำเป็นตรงนี้เลย ไม่พึ่ง style.css อย่างเดียว — กันเคส ST บางเครื่องโหลดไฟล์ css จาก repo ไม่สำเร็จ
 // แล้วปุ่มไม่มีตำแหน่ง/ไม่โผล่ให้เห็นเลย (ปัญหาที่เจอบ่อยสุดของการติดตั้ง extension แบบนี้)
 fab.style.cssText = 'position:fixed;right:18px;bottom:90px;width:52px;height:52px;border-radius:50%;'
  + 'background:linear-gradient(160deg,#3a3a3c,#1c1c1e);box-shadow:0 6px 18px rgba(0,0,0,.45);'
  + 'display:flex;align-items:center;justify-content:center;z-index:2147483000;cursor:pointer;'
 ...
 // ★ เช่นกัน: position/z-index/display เป็น inline style ล้วน ไม่พึ่ง class ".open" จาก style.css อย่างเดียว
 overlay.style.cssText = 'position:fixed;inset:0;z-index:2147483001;display:none;align-items:center;'
 ...
 setInterval(() => { if (overlay.style.display !== 'none') render(); }, 15000);

 console.log('[myphone] ปุ่มลอยถูกสร้างแล้ว (ตำแหน่งฝังในโค้ดโดยตรง ไม่พึ่ง style.css)');
}
...
/* ============================================================
   จุดเข้าใช้งานสำรอง (กันเคสปุ่มลอยไม่โผล่ด้วยเหตุผลอะไรก็ตาม)
   1) รายการในเมนูไม้กายสิทธิ์ ✨ ของ ST เอง (#extensionsMenu)
   2) ปุ่ม "เปิด MyPhone" ในหน้า Extensions settings ของ ST เอง
   ทั้งสองจุดนี้ยืมคลาส CSS ของ ST เอง (list-group-item, inline-drawer, menu_button)
   เพื่อให้หน้าตากลมกลืนกับเมนูจริงของ ST ไม่ใช่การลอกสไตล์จาก extension อื่น
   ============================================================ */
function injectWandButton() {
 if (document.getElementById('mp-wand-btn')) return true;
 ...
 item.tabIndex = 0;
 item.innerHTML = `<div style="width:20px;display:flex;justify-content:center">${MP_ICON.phone}</div><span>MyPhone</span>`;
...
function registerSettingsPanel() {
 ...
 if (!host || document.getElementById('mp-ext-drawer')) return false;
 host.insertAdjacentHTML('beforeend', `
<div id="mp-ext-drawer" class="inline-drawer">
 <div class="inline-drawer-toggle inline-drawer-header">
  <b>MyPhone</b>
  <div class="inline-drawer-icon fa-solid fa-circle-chevron-down down"></div>
 </div>
 <div class="inline-drawer-content">
  <div style="font-size:12px;opacity:.7;margin-bottom:8px">version <b>${MP_VERSION}</b></div>
  <div style="font-size:12px;opacity:.7;margin-bottom:8px">เปิดได้จากปุ่มลอยมุมขวาล่าง เมนูไม้กายสิทธิ์ ✨ หรือปุ่มด้านล่างนี้</div>
  <input id="mp-ext-open" class="menu_button" type="button" value="เปิด MyPhone">
 </div>
</div>`);
 document.getElementById('mp-ext-open')?.addEventListener('click', openPhone);
 return true;
}

/* ============================================================
   บูตแบบรอ "จุดยึด" ของ ST ให้พร้อมก่อนจริง ๆ ค่อยแปะปุ่ม (กันโดนล้างทิ้งถ้า ST ยัง
   re-render หน้าอยู่ตอนสคริปต์เรารันครั้งแรก) + MutationObserver คอยเช็คว่าปุ่มหายไปไหม
   ถ้าหายก็แปะใหม่ให้เอง — วิธีนี้อ้างอิงแพทเทิร์นที่ยืนยันแล้วว่าใช้ได้จริงกับ ST
   ============================================================ */
console.log(`[myphone] index.js เริ่มอ่านโค้ดแล้ว (v${MP_VERSION})`);
...
(function boot() {
 ...
 const timer = setInterval(() => {
  ...
  if (host) {
   ...
   try {
    ...
    hookMainChatEvents();
    console.log(`[myphone] v${MP_VERSION} โหลดสำเร็จ พร้อมใช้งาน (พบจุดยึดหลังรอ ${tries * 0.5} วิ)`);
   } catch (e) {
    console.error('[myphone] เกิดข้อผิดพลาดตอนเริ่มต้น extension:', e);
   }
  } else if (tries > 60) {
   clearInterval(timer);
   console.warn('[myphone] ไม่พบจุด mount ของ ST หลัง 30 วิ — ลองรีเฟรชหน้าเว็บใหม่');
  }
 }, 500);
 ...
 try {
  const mo = new MutationObserver(() => {
   if (!document.getElementById('myphone-fab')) buildSkeleton();
   if (!document.getElementById('mp-wand-btn')) injectWandButton();
  });
  mo.observe(document.body, { childList: true, subtree: false });
 } catch (e) { console.warn('[myphone] ตั้ง MutationObserver ไม่สำเร็จ', e); }
})();
